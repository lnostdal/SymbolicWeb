...
(defn dao-db-array-to-clj-vector "DB (SQL Array) --> SW (Clj Vector)"
  [^java.sql.Array db-array]
  (vec (.getArray db-array)))


(defn dao-clj-coll-to-db-array
  "SW (Clj Coll) --> DB (SQL Array)"
  ^String [clj-coll ^String db-type]
  (cl-format false "ARRAY[~{~S~^, ~}]::~A[]" clj-coll db-type))


(defn dao-clj-cm-to-db-array "SW (ContainerModel) --> DB (SQL Array)"
  ^String [^ContainerModel container-model ^String db-type]
  (let [elts (atom [])]
...
(defn dao-default-clj-to-db-transformer "SW --> DB"
  [m]
  (assoc m
    :key (cond
           (not (:key m))
...
(defn dao-clj-to-db-transformer "SW --> DB"
  [^DBCache db-cache ^Ref obj ^Keyword clj-key clj-value]
...
(defn dao-default-db-to-clj-transformer "DB --> SW"
  [m]
  (assoc m
...
(defn dao-db-to-clj-transformer "DB --> SW"
  [^DBCache db-cache ^Ref obj ^Keyword db-key db-value]
...
(defn dao-db-to-clj-handler "DB --> SW"
  [^DBCache db-cache ^Ref obj ^Keyword db-key db-value ^Boolean ensure-persistent?]
...
(defn dao-db-to-clj-entry-handler "DB --> SW
  ENTRY-DATA: Map representing a DB Row; DB-KEY and DB-VALUEs."
  [^DBCache db-cache ^Ref obj entry-data ^Boolean ensure-persistent?]
  (doseq [[^Keyword db-key db-value] entry-data]
    (dao-db-to-clj-handler db-cache obj db-key db-value ensure-persistent?)))



(defn dao-ensure-persistent-vm-field "Sets up reactive SQL UPDATEs for VALUE-MODEL."
  [^DBCache db-cache ^Ref obj ^Keyword clj-key ^ValueModel value-model]
...
(defn dao-db-value-to-vm-handler "DB --> SW
  Returns the ValueModel."
  ^ValueModel [^DBCache db-cache ^Ref obj ^Keyword clj-key db-value]
...
(declare dao-get)
(defn dao-db-value-to-cm-handler "DB --> SW
  Returns the ContainerModel"
  ^ContainerModel [^DBCache db-cache ^Ref obj ^Keyword clj-key ^java.sql.Array db-value
...
(defn dao-backend-get "Used by DB-GET; see DB-GET.
  Returns OBJ or NIL"
  ^Ref [^DBCache db-cache ^Long id ^Ref obj]
...
(defn dao-cache-put "Store association between ID and OBJ in DB-CACHE.
  If ID already exists, the entry will be overwritten."
  [^DBCache db-cache ^Long id ^Ref obj]
  (let [id (long id)] ;; Because (.equals (int 261) 261) => false
    (.put (get-internal-cache db-cache) id obj)))



(defn dao-cache-remove "Removes object based on ID from DB-CACHE."
  [^DBCache db-cache ^Long id]
...
(defn dao-remove "SQL `DELETE FROM ...'."
  [^Ref obj ^String table-name]
