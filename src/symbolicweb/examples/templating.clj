...
(defn mk-templates-viewport [request session]
  (let [root-widget (mk-bte :id "_body" :root-widget? true)
    ...
    (jqAppend root-widget
      (whc [:div]
        ...
        [:p (sw (mk-HTMLCTemplate template
                                  (fn [template-widget]
                                    [;;".sw-template" template-widget
...
(defmethod symbolicweb.examples.nostdal-org/mk-nostdal-org-viewport "/templates" [uri request session]
  (mk-templates-viewport request session))
